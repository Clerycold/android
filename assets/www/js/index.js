...
var app = {
        ...
        */
        ...
        socket.on('connect',function(){  });
            ...
            socket.on('marquee_'+device.uuid,function(msg){
                ...
                app._getAnnouncementMarqueeList(function(datas){
                    if(datas !='error'){
                        $.each(datas, function (i, v) {
                            ...
                            domStr+= v.id
                            domStr+='"/>';
                            domStr+='<input type="hidden" value="'+ v.read +'" />';
                            domStr+= v.title
                            domStr+='</li>';
                        });
                    }
                ...
                var speed = 3000 + scrollSpeed;
                 ...
                 if(JSON.parse(msg).length >1 || (datas.length >1 && datas !='error')){
        ...
        for(var i=0;i<val.length;i++){
            $.each(com, function (idx, v) {
                if(v.id==val[i]){
                    ...
                    dom+='<a>';
                    dom+='<div class="name build">';
                    dom+='<div class="build-img" style="background:url(http://202.39.212.221/communitylogo/'+ val[i] +'.jpg);background-position:center center; background-size:cover;">';
                    dom+='</div>';
                    dom+='<div class="info">';
                    dom+='<h4><span>我的社區</span>'+ v.community +'</h4>';
                    dom+='<p>社區基本資料、合作店家資訊與介紹</p>';
                    dom+='</div>';
                    dom+='</div>';
                    dom+='</a>';
                    dom+='</li>';
    ...
    _getStoresInfo:function(com,type,callback){

        $.ajax({
            dataType: 'json',
            url: app._getWebApi('Store','getStoreList'),
            type: 'GET',
            data: 'com_id='+com+'&s_type='+type,
            async: true,
            success: function (datas) {
                callback(datas);
            },error:function(e){
    ...
    _getAnnouncementMarqueeList:function(callback){
        var comId='';
        if(app._getLocalStorageJson('com_count') > 0){
          ...
          comId=comId.substring(0,comId.length-1);
           $.ajax({
                dataType: 'json',
                url: app._getWebApi('Announcement','getAnnouncementMarquee'),
                type: 'GET',
                data: 'com_id='+comId+'&code='+this._getLocalStorageJson('code'),
                async: true,
                success: function (datas) {
                    callback(datas);
                },
   ...
   // } 
    ...
    _getEasyLifeList:function(callback){
        if(generalObject.key=='easyLife'){
            callback(generalObject.value);
        }else{
            $.ajax({
                dataType: 'json',
                url: app._getWebApi('Store','getEasyLifeList'),
                type: 'GET',
                async: true,
                success: function (datas) {
                    generalObject.key='easyLife';
                    generalObject.value=datas;
                    callback(datas);
                },error:function(e){
                    callback('error');
                }
    ...
    _getCommunityInfo:function(com,callback){

        $.ajax({
            dataType: 'json',
            url: app._getWebApi('Store','getCommunityInfo'),
            type: 'GET',
            data: 'com_id='+com,
            async: true,
            success: function (datas) {
    ...
    _footerShow:function(p,id){
        ...
        footerId.on('click','#footer ul li:eq(0) a',function(){
            ...
            if(p !='moreList.html'){
                ...
                _mobile.navigate("moreList.html", { changeHash: true });
            }          
        })
