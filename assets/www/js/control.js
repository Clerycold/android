...
function _onBackPageLogin(){
     ...
     if(temp1 !='' && temp1 !='new'){    
        ...
        _mobile.navigate('index.html', { changeHash: true,reverse:true});
     }
}
...
function _onBackPageAd(){
     ...
     var temp1=tempVal;
     tempVal='';
     _mobile.navigate(temp1, { changeHash: true,reverse:true});
}
...
function _onBackPageSn(){
     ...
     _mobile.navigate('login.html', { changeHash: true,reverse:true});
}
function _onBackPageGasPrice(){
    document.removeEventListener("backbutton", _onBackPageGasPrice, false);
    _mobile.navigate('easylife.html', { changeHash: true,reverse:true});
}
...
function _onBackPageSystemMsg(){
    ...
    storeObject.someValue='system';
    _mobile.navigate("myMsg.html", { changeHash: true,reverse:true});
}
function _onBackPageUserMsg(){
   document.removeEventListener("backbutton", _onBackPageUserMsg, false);
    _mobile.navigate("myMsg.html", { changeHash: true,reverse:true});
}
function _onBackPageStore(){
   document.removeEventListener("backbutton", _onBackPageStore, false);
...
$(function () {
   ...
   app._ga_trackView('snLogin','');
    ...
    _index.on('click', '.guide .swiper-container a,#guide_close', function () {
      ...
      var id=$(this).find('input').val();
       storeObject.someValue=id;
       storeObject.anotherValue='index.html';
      _mobile.navigate('adDetail.html', { changeHash: true});
        ...
        if(type==0){
            storeObject.someValue=link;
            storeObject.anotherValue='index.html';
            _mobile.navigate('adDetail.html', { changeHash: true});
        }else if(type==1){
            var ref= window.open(link, "_blank");
        }else{
            if(type.length==8){ //公告資訊
                if(this.querySelectorAll('input')[2].value==''){
                    if(_annBadgeCount >0){
                        _annBadgeCount--;    
                        this.querySelectorAll('input')[2].value='read';  
                    }
                }
                storeObject.someValue=link;
                storeObject.anotherValue=type;
                _mobile.navigate("announcementDetail.html", { changeHash: true, data: { 'paremeter': link,'com': type,'height':0} });   
            }else{
                app._dialog(this.innerText);
            }
        }
      ...
      if(app._getLocalStorageJson('com_count')==0){
       ...
       if(app._getLocalStorageJson('com_count')==0){
            ...
            storeObject.someValue='index.html';
            _mobile.navigate('myMsg.html', { changeHash: true});
      }
       ...
       if(app._getLocalStorageJson('com_count')==0){
        ...
        _mobile.navigate('profile.html', { changeHash: true});
    }).on('click','#easylife_btn', function(){
         app._ga_trackEvent('master','easylife','生活便利通',1);
         _mobile.navigate('easylife.html', { changeHash: true});
  ...
  document.removeEventListener('backbutton',_onBackPageList,false);
   ...
   app._ga_trackView('collectionPayment','');
          ...
          if(fileBtn.find('.file-tag').length>0){
            ...
            if(type=='ad'){
             ...
             if(linkType ==0){
                ...
                if(linkType==3){
                    storeObject.someValue=fileBtn.find('input:eq(2)').val();
                    storeObject.anotherValue='myMsg.html';
                    _mobile.navigate('adDetail.html', { changeHash: true});
                }else{
    ...
    var mymsg=$(this);
    app._getNotificationRecord(mymsg,storeObject.someValue);
...
app._dialog('已複制連結');
        ...
        if(val.find('input[type^=hidden]:eq(1)').val() ==""){
            ...
            storeObject.someValue=val.find('input[type^=hidden]:eq(1)').val();
            storeObject.anotherValue='moreList.html';
             _mobile.navigate('adDetail.html', { changeHash: true }); 
        }
 ...
 // var val=app._getUrlParameters($(this).data("url"),'paremeter');
  ...
  app._ga_trackView('more-ks','');
    ...
    more.on('click','#header a:eq(0)',function(){
        if(next =='adDetail.html' && val !=''){
            storeObject.someValue=val;
            storeObject.anotherValue='index.html';
            _mobile.navigate(next, { changeHash: true,reverse:true});
        }else{
            _mobile.changePage('moreList.html', { changeHash: true,reverse:true});
        }         
    }).on('click','#header a:eq(1)',function(){
  ...
  iframe.style.height=_tmpHeight-this.querySelector('#header').offsetHeight +'px';
    ...
    var next=storeObject.anotherValue;
    var s_ad=storeObject.someValue;
    tempVal=next;
...
document.removeEventListener("backbutton",_onBackPageAnn, false);
    ...
    page.on('click', '#header a:eq(0)', function () {
        ...
        storeObject.someValue=$(this).find('input[type^=hidden]').val();
        storeObject.anotherValue=com_id;
        ...
        //_ann.querySelector('#ann_list').innerHTML='';
         //app._loadingShow();
        _mobile.navigate("announcementDetail.html", { changeHash: true });  
    }).on('click','#header a:eq(1)',function(){
        app._setLocalStorage('annNews','');
        g_typeName=[];
        g_typeArr=[];
   ...
   if(app._getLocalStorage('annNews')=='' || JSON.parse(app._getLocalStorage('annNews')).com !=com_id){ 
     app._getAnnouncementList(com_id,function(data){
      ...
      //setTimeout(function(){
          //$.mobile.silentScroll(storePosition.topCoordinate);
           _htmlBody.animate({scrollTop:storePosition.topCoordinate},0);
          app._loadingHide(100);
        //},300);
   ...
   app._ga_trackView('announcement','');
      ...
      var page=$(this);
      var com_id=storeObject.anotherValue;
      var detail_id=storeObject.someValue;
      var image;
      ...
      $.ajax({
        ...
        success: function(datas){
            $.each(datas, function (i, v) {
               page.find('#annDetail_title').text(v.title);
               // $('#annDetail_sub').text(v.create_date +' '+ v.type);
               page.find('#annDetail_sub').html(v.create_date +' <span>'+ v.type + '</span>');
               page.find('#annDetail_txt').html(v.detail);
               if(v.image!=''){
                    image=v.image;
    ...
    var com=storeObject.anotherValue; 
    $(this).on('click', '#header a', function () {
    ...
    var page=$(this);
    var com=storeObject.anotherValue;
    var msg_id=storeObject.someValue; 
    ...
    */
      $(this).on('click', '#header a:eq(0)', function () {
         if(app._getLocalStorageJson('com_count') =='1')
         {
            ...
            storeObject.someValue=par;
            _mobile.navigate("feedbackEdit_v2.html", { changeHash: true});
      }).on('click','.feedback-content ul li a',function(){
            storeObject.someValue=$(this).find('input[type^=hidden]').val();
            storeObject.anotherValue=par;
            _mobile.navigate("feedbackDetail.html", { changeHash: true });
      });
  ...
  var resHistory=$(this);
    ...
    resHistory.on('click','#header a:eq(0)',function(){
      if(backpage==com_id){
        ...
        document.removeEventListener('backbutton',_onBackPageSystemMsg,false);
        storeObject.someValue=backpage.split('_')[1];
        _mobile.navigate("myMsg.html", { changeHash: true,reverse:true});
      }
...
app._ga_trackView('reservationHistory','');
    ...
    profile.on('click', '.listview:eq(0) ul li', function () {
      ...
      if (window.cordova && window.cordova.plugins.settings) {
        ...
        if(this.id=='no_login'){
            _mobile.navigate('login.html', { changeHash: true});
        }else{
           //app._dialog('功能尚未開放。'); 
    ...
    pp.on('click', '#header a',function(){
         _mobile.navigate('profile.html', { changeHash: true, reverse: true });
    })
   ...
   $(this).on('click','#header a:eq(0)',function(){
    ...
    $(this).on('click','#header a:eq(0)',function(){
         ...
         _mobile.navigate('gasStationList.html', { changeHash: true });
    });
  ...
  _map.setClickable(true); 
    ...
    easylife.on('click','#header a:eq(0)',function(){
         _mobile.changePage('index.html', { changeHash: true, reverse: true });
    }).on('click','.list li.store',function(){
        storeObject.someValue=this.querySelector('input').value;
         window.ga.trackEvent('easylife', 'store', '社區商家', 1);        
        _mobile.navigate('myCommunity.html', { changeHash: true});
    }).on('click','#easy_gas',function(){  
       app._ga_trackEvent('easyLife','gasPrice','油價',1);
            _mobile.navigate('gasStationPrice.html', { changeHash: true });
    }).on('click','#easy_map',function(){  
       app._ga_trackEvent('easyLife','gasMapList','油價',1);
         _mobile.navigate('gasStationList.html', { changeHash: true });  
    }).on('click','.list li.easy',function(){  
        var val=this.querySelectorAll('input');

        if(val[0].value==0){
            var ref=window.open(val[1].value,'_blank');
        }else{
            if(val[2].value !=''){
                storeObject.someValue=val[2].value;
                storeObject.anotherValue=val[1].value+'.html';
                 _mobile.navigate('adDetail.html', { changeHash: true});  
            }else{
                _mobile.navigate(val[1].value+'.html',{changeHash: true});
            }
        }
    }).on('click','#df',function(){  
       app._ga_trackEvent('easyLife','df','大豐環保',1);
       storeObject.someValue='17020101';
       storeObject.anotherValue='easylife.html';
        _mobile.navigate('adDetail.html', { changeHash: true});          
    });
    ...
    app._footerShow('easylife.html',this.id);
      ...
      var checkEasy=false;
      app._getEasyLifeList(function(datas){
        if(datas !='error'){
             $.each(datas, function (i, v) { 
                domStr+='<li class="easy">';
                domStr+='<input type="hidden" value="'+ v.link_type +'" />';
                domStr+='<input type="hidden" value="'+ v.link +'" />';
                domStr+='<input type="hidden" value="'+ v.bd_id +'" />';
                domStr+='<a>';
                domStr+='<div class="name">';
                domStr+='<img src="http://202.39.212.221/CHK_APP/sub_icon/'+ v.img +'" />';
                domStr+='<div class="info">';
                domStr+='<h4>'+v.title+'</h4>';
                domStr+='<p>'+v.note+'</p>';
                domStr+='</div>';
                domStr+='</div>';
                domStr+='</a>';
             ...
             easylife.find('ul.list').append(domStr);
             if(checkStore==true){
...
}).on('pagebeforecreate', '#myCommunityStore', function (e) {
    app._loadingShow();
}).on('pageinit', '#myCommunityStore', function (e) {
    var myCom=$(this);
    var com = storeObject.someValue;
    myCom.on('click','#header a',function(){       
        _mobile.navigate('myCommunity.html', { changeHash: true,reverse:true});
        ...
        if(input[0].value !='' && input[1].value !=''){
            options = {
              'title': '請選擇開啟方式?',
              'buttonLabels': ['撥號('+ input[0].value+")" , '撥號('+ input[1].value+")" ,'地圖資訊','商店內容'],
              'addCancelButtonWithLabel': '取消'
            };
             window.plugins.actionsheet.show(options, function(buttonIndex){
                switch(buttonIndex){
                    case 1:
                       var ref= window.open('tel:'+input[0].value.replace('-',''), '_system');
                    break;
                    case 2:
                       var ref= window.open('tel:'+input[1].value.replace('-',''), '_system');
                    break;
                    case 3:
                       var ref= window.open("http://maps.google.com/?q="+ input[2].value,"_blank","location=no");
                    break;
                    case 4:                   
                       navigator.notification.alert(
                             input[3].value,
            ...
            var phone= input[0].value !='' ? input[0].value :input[1].value;
            options = {
              'title': '請選擇開啟方式?',
              'buttonLabels': ['撥號('+ phone+")" ,'地圖資訊','商店內容'],
              'addCancelButtonWithLabel': '取消'
            };
            window.plugins.actionsheet.show(options, function(buttonIndex){
                switch(buttonIndex){
                    case 1:
                       var ref= window.open('tel:'+phone.replace('-',''), '_system');
                    break;
                    case 2:
                       var ref= window.open("http://maps.google.com/?q="+ input[2].value,"_system");
                    break;
                    case 3:                   
                       app._dialog(input[3].value);
                    break;
                    default:
...
}).on('pageshow', '#myCommunityStore', function (e) {
    var myCom=$(this);
    var com = storeObject.someValue;
    var type = storeObject.anotherValue;
    var domStr='';
    app._getStoresInfo(com,type,function(datas){
        $.each(datas, function (idx, v) { 
           domStr+='<li>';
           domStr+='<input type="hidden" value="'+ v.s_tel +'" />';
           domStr+='<input type="hidden" value="'+ v.s_phone +'" />';
           domStr+='<input type="hidden" value="'+ v.s_city+v.s_area+v.s_address +'" />';
           domStr+='<input type="hidden" value="'+ v.s_content +'" />';
           domStr+='<a>';
           domStr+='<div class="info">';
           domStr+='<h4>'+ v.s_name +'</h4>';
           domStr+=v.s_subtitle != '' ? '<p>'+ v.s_subtitle +'</p>':'';
           if(v.s_phone !='' || v.s_tel != ''){
               domStr+='<p>';
               if(v.s_phone !=''){
                    domStr+= v.s_phone;
               }
               if(v.s_tel !=''){
                    domStr+=v.s_phone != '' ? '、'+v.s_tel :v.s_tel;
               }
               
               domStr+='</p>';                      
           }
           domStr+=v.s_address != '' ? '<p>'+ v.s_city+v.s_area+v.s_address +'</p>':'';
           domStr+='</div>';
           domStr+='</a>';
           domStr+='</li>';
       });
        myCom.find('ul').append(domStr);
        app._loadingHide();
    });
}).on('pagebeforecreate', '#mycommunity_v2', function (e) {
    app._loadingShow();
}).on('pagebeforehide', '#mycommunity_v2', function (e) {
    document.removeEventListener('backbutton',_onBackPageStore,false);
}).on('pageinit', '#mycommunity_v2', function (e) {
    document.addEventListener('backbutton',_onBackPageStore,false);
    this.style.height=_tmpHeight+20+'px';
    var myCom=$(this);
    var com = storeObject.someValue;
    //com=JSON.parse(app._getLocalStorage('comInfo')).info[0].id;
        myCom.find('img').attr('src','http://202.39.212.221/communitylogo/'+ com +'.jpg');
      myCom.on('click','.infoWrap ul.storeCgy li',function(){
        storeObject.anotherValue=this.querySelector('input').value;
        _mobile.navigate("myCommunityStore.html", { changeHash: true});
      }).on('click','#header a',function(){
            _mobile.navigate("easylife.html", { changeHash: true, reverse: true });
      });
...
}).on('pageshow', '#mycommunity_v2', function (e) {
    var name=['餐飲','消費購物','居家生活','交通','教育','娛樂'];
    var type=['foods','shopping-bag','house','car','edu','fun'];
    var myCom=$(this);

    var com = storeObject.someValue;
    //this.querySelector('.mycommunity_v2-content').style.height=_tmpHeight+20+'px';
     app._getCommunityInfo(com,function(status){
        if(status !='error'){
            var _li;
            var _a;
            var _p;
            var _i;
            myCom.find('ul.basic').html('');
            $.each(status, function (idx, v) { 

                if(idx==0){
                    var _arr=[v.address,v.tel];
                    for(var i=0;i<_arr.length;i++){
                        _li=$('<li />');
                        _a= i==0 ? $('<a />',{'onclick':'window.open("http://maps.google.com/?q='+ _arr[i]+'","_system");'}):$('<a />',{'href':'tel:'+ _arr[i]});
                        _p=$('<p />',{'text':_arr[i]});
                        _i= i==0 ? $('<i />',{'class':'icon-pin'}):$('<i />',{'class':'icon-phone'});
                        _a.append(_p).append(_i);
                        _li.append(_a);
                        myCom.find('ul.basic').append(_li);
                    }                  
                    if(v.fax !=''){
                        _li=$('<li />',{'class':'fax'});
                        _a=$('<a />');
                        _span=$('<span />',{'class':'subtitle','text':'傳真'});
                        _a.append(_span);
                        _span=$('<span />',{'class':'subtxt','text':v.fax});
                        _a.append(_span);
                        _li.append(_a);
                         myCom.find('ul.basic').append(_li);                   
                    }
                    myCom.find('h3').text(v.community).siblings().text(v.construction);
                }
                $.each(v.storeType, function (index, val) { 
                    _li=$('<li />',{'class':type[val.s_type]});
                    _a=$('<a />');
                    _span=$('<span />',{'class':'name','text':name[val.s_type]});
                    _span2=$('<span />',{'text':'('+ val.total +')'})
                    _span.append(_span2);
                    _i=$('<i />',{'class':'icon-'+type[val.s_type]});
                    _a.append(_i).append(_span);
                    _li.append($('<input >',{'type':'hidden','value':val.s_type})).append(_a); 
                    myCom.find('ul.storeCgy').append(_li);   
                });  
            });
        }else{
            app._dialog('網路連線異常，請再重新嘗試連線');
            //_mobile.changePage('moreList.html', { changeHash: true,reverse:true});
        }
        app._loadingHide();
    });
